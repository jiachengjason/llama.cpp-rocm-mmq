...
#pragma unroll
    ...
    if constexpr (!has_fusion) {
        GGML_UNUSED_VARS(use_gate, use_bias, use_residual, use_gate_bias, use_scale, use_gate_scale,
                active_glu, gate_bias, x_bias, x_residual, x_scale, gate_scale, tmp_gate);
    }
    if constexpr (type != GGML_TYPE_NVFP4) {
...
static void mul_mat_vec_q_switch_fusion(
    ...
    const bool has_fusion = fusion.gate != nullptr || fusion.x_bias != nullptr || fusion.gate_bias != nullptr ||
                            fusion.x_residual != nullptr || fusion.x_scale != nullptr || fusion.gate_scale != nullptr;
